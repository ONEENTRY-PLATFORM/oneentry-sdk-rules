...
export default async function ProductPage({
  ...
  const attrs = product.attributeValues || {};
  // image (1 file → object, 2+ → array) and text (always array) — capture wrapper universally
  const rawPic = attrs.pic?.value;
  const imageUrl = (Array.isArray(rawPic) ? rawPic[0] : rawPic)?.downloadLink || '';
  const title = product.localizeInfos?.title || '';
  const price = attrs.price?.value ?? 0;   // number or null (v1.0.157)
  const rawDesc = attrs.description?.value;
        ...
        <section>
          ...
          <div className="grid grid-cols-4 gap-4">
            {relatedProducts.map((p) => {
              ...
              const rawImg = a.pic?.value;
              const img = (Array.isArray(rawImg) ? rawImg[0] : rawImg)?.downloadLink || '';
              return (
                <div key={p.id}>
