...
# Uploading Images via the Internal Admin API (with Previews/LQIP)

Use this skill for programmatically uploading images to the `image` / `groupOfImages` attributes of pages, products, and admins in OneEntry. The main trap is the request parameters: without `&template=1`, the preview (`previewLink`) **will never be created**.

> Base: rule `admin-api` (login, Bearer, auto-save the whole object) and skill `/admin-fill-content` (common module `login()`).
...
- **`template=1` is mandatory** — it enables the generation of previews. Without it, the response comes without `previewLink`, and the preview will not appear either asynchronously or during a subsequent PUT (experimentally verified). Just `entity=images` is not enough.
...
Place the **entire object from the upload response** (including `previewLink` and `defaultPreview`) + `params: {isImageCompressed: true}` in the PUT. The internal key (`image_id{N}` / `groupOfImages_id{N}`) — from the GET entity, do not guess.

## Multipart Gotcha

❌ Do not set a global `content-type: application/json` in the request-context Playwright — it will break multipart. Only pin `authorization`; Playwright will set `content-type` per-request itself.

## Audit and Re-upload

- Check which files do not have previews: go through the entities with GET and see if there is a `previewLink` for the file objects.
...
- No endpoint for deleting files in the admin API has been found — old files remain on the CDN as orphans; this is expected.

## How to Capture the Exact Recipe from the UI (if something doesn't match)

Upload zones in the admin panel: page — `/content/edit-page/{id}?tab=3` (Attributes tab), admin — `/admins/edit-page/{id}?tab=2`; inputs — `input#image_id{N}` / `input#groupOfImages_id{N}`. Upload a file manually and intercept the request (Playwright `browser_network_requests` → `browser_network_request`) — the exact query parameters will be visible there. Note: the UI for some types (e.g., `type=admin`) sends the request **without** `template` — meaning the admin panel does not create previews in those places; `template=1` still works there, so always add it.

## Frontend: How to Use LQIP

`previewLink.default[0]` → `blurDataURL` + `placeholder="blur"` in `next/image`. For small avatars (≤48 px), the blur is not visible and only inflates the base64 markup — consciously avoid using it there.
