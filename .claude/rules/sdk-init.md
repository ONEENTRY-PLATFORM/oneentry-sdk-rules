<!-- META
type: rules
fileName: sdk-init.md
rulePaths: ["lib/oneentry.ts","lib/oneentry/**/*.ts"]
paths:
  - "lib/oneentry.ts"
  - "lib/oneentry/**/*.ts"
-->
...
The minimal set above is enough for a demo; in a real project, each of these five addresses its class of bugs.
...
- **`getApiSafe()`** — a non-throwing version of `getApi()`, returns `null`. Two entry points instead of one: `getApi()` will throw loudly (misconfiguration of deployment is immediately visible), `getApiSafe()` will return `null` on degradation paths — the showcase renders without CMS. Detailed information about degradation — `.claude/rules/error-handling.md`.
- **`createRequestApi({ langCode?, guestId?, deviceMetadata? })`** — a one-time instance that does not touch the singleton. Always needed when a server request speaks on behalf of **one specific** visitor: OAuth code exchange with browser fingerprint, guest call with `x-guest-id`. The absence of such a helper pushes towards `setDeviceMetadata()`/`setGuestId()` on the shared singleton — which will apply to all parallel visitors of the same Node process.
- **`hasStoredSession()`** — "does this browser have a session": `hasActiveSession()` or a non-empty `localStorage['refresh-token']`. Client-side replacement for checking auth-cookie: user-scoped calls do not trigger a known 401. Always `false` on the server.
...
The server singleton is shared among all visitors of the process. Setting someone else's `refreshToken` there is a leak of `Authorization` between requests: the next visitor will receive the previous one's data. The rule about `setGuestId`/`setDeviceMetadata` ("only per-request instance") also applies to `reDefine` — but silently, because the call itself does not fail.
...
export async function reDefine(refreshToken: string, langCode?: string): Promise<void> {
  ...
  if (typeof window === 'undefined') {
    throw new Error('reDefine() — browser only: the server singleton is shared among all visitors.')
  }
...
**⚠️ Check hasActiveSession before calling reDefine:**
...
// ❌ EXCESSIVE — blind reDefine without check
...
The option `traficLimit: true` (exactly like this, with one "f") in the config disables the SDK's automatic enrichment requests: loading products of blocks in `Blocks.getBlocks`/`getBlockByMarker` and `Pages.getBlocksByPageUrl`, loading full entities in `Pages.searchPage`/`Products.searchProduct`. The default is `false` (enrichment is enabled). The SDK's JSDoc describes the semantics inversely — in the code, traffic is saved by `true`.

⚠️ Enabled `traficLimit` changes the **form** of the quick search result, not just the volume of traffic: `searchProduct` returns `IProductSearchResult[]` (`{ id, title, pageId }`), `searchPage` — `IPageSearchResult[]` (`{ id, title }`), without `attributeValues`, `localizeInfos`, and `blocks`. From v1.0.157 this is reflected in types (`IProductsEntity[] | IProductSearchResult[] | IError`), earlier the signature promised a full entity in both modes, and the code silently received `undefined`. If you need details — load them by id (`getProductsByIds`).
...
const results = await getApi().Products.searchProduct(query); // IProductsEntity[] (when traficLimit — IProductSearchResult[])
```
...
**In the browser — no configuration is needed.** If `guestId` is not set, the SDK generates a stable id on the first request (via Web Crypto) and saves it in `localStorage` under the key `oneentry_guest_id`. It survives reloads — the guest cart is not lost.

**On the server, the SDK does NOT generate guest id automatically.** One shared instance of `defineOneEntry` serves all visitors — auto-generation would lead to one guest cart leaking to everyone. Therefore, on the server, **pass `guestId` explicitly** for each visitor (for example, from a cookie):
...
// (it is NOT on the root object of defineOneEntry); state is shared — applies to the entire instance
...
> Pattern for Next.js: generate an id (`crypto.randomUUID()`) on the first visit, place it in an `httpOnly` cookie, and on the server pass it to `Users.setGuestId()` before guest calls to cart/wishlist. After login — the guest cart is usually "merged" into the user cart via `Users.setCart()` and reset `Users.setGuestId('')`.

## Device Metadata (override fingerprint, SDK ≥ 1.0.155)

The API ties refresh tokens to the header `x-device-metadata` (device fingerprint); the SDK sends it on POST requests and on `/refresh`. By default, the string is computed from the environment in which the SDK operates — on the server, it is the Node fingerprint, not the browser one.

- `deviceMetadata` in the `defineOneEntry` config — explicitly set the header string;
