...
The rule about **choosing** an entity and attribute type when you create the structure in the OneEntry project (via script through admin API or by advising the user what to create in the admin panel). How to read what has already been created — `.claude/rules/attribute-values.md`; how to write through `/api/admin/*` — `.claude/rules/admin-api.md` and skill `/admin-fill-content`.

> Don't guess: before designing, check what already exists in the project via `/inspect-api`. Half of the "needed" entities are usually already created under a different marker.
...
2. **Who inputs the data?** Editor → `attributeValues` (Page, Block, Product, Admin). Visitor → `formData` (FormData, Order, User). These are different formats and different modules; user input placed in attributes can only be created later through the admin API.
...
| Cart, favorites | Server-side cart and wishlist for the user — no need to create entities, they work for guests too |
| Team, masters, doctors | Admins with a designated set of attributes |
| Working hours, booking slots | Attribute `timeInterval` |
| Promotions, coupons, bonuses | Discounts (conditions configured in the admin panel, not in code) |
...
**Block — a reusable entity.** It exists so that content can be displayed in several places while being edited in one: footer, persistent banner, contact block. Linking a block to a page can be done from either side: the block has a **Linked pages** tab (page tree with checkboxes), and the page has a **Blocks** tab with a **Block selection** field. When a page is assembled from blocks, the **Blocks** tab is more convenient: the entire composition of the page is visible and organized in one list.

- content belongs to one page → **page attributes**;
- content is repeated on several pages and should change at once → **block**.

A banner created as fields of each page will have to be edited as many times as there are pages. Details on output — `.claude/rules/pages-blocks.md`.

## Attribute Sets

**A set is created for a record type, not for a record.** Five similar banners should share one set — otherwise, editing the structure turns into five edits.

The type of the set is chosen at creation and limits its application: `forPages`, `forBlocks`, `forProducts`, `forForms`, `forUsers`, `system`. The `system` set is not tied to anything — it is a storage for settings and UI dictionaries.
...
19 types. Below is what each is responsible for; value forms when reading — `.claude/rules/attribute-values.md`.
...
| `integer` | Integer: stock, capacity, year, priority |
...
| `time` | Only time, repeating day after day: "delivery from 9:00" |
| `image` | One image: cover, logo, background (widget is limited to one file) |
| `groupOfImages` | Gallery, photo album — always a collection; the editor fills Alt and Title for each file |
...
| `timeInterval` | Schedule as a rule: booking and delivery slots, working hours |
...
- **`text` / `textWithHeader`.** As soon as there is more than one homogeneous item — it is a repeater. You cannot create `faq_q1`, `faq_a1`, `faq_q2`: the quantity is fixed by the schema, the editor will not add an eleventh item, and the front parses numbered markers.
...
For the front, the main takeaway is: a mandatory field is marked with **an asterisk next to the label**, and the indicator is taken from `validators.requiredValidator`, not from the list of markers in the code. Thus, the mandatory setting enabled in the admin panel appears on the site immediately, without editing the component.
...
7. **Attribute `rating` instead of aggregate** — the final rating comes in `entity.rating`, an attribute with such a marker is usually legacy and returns a hardcoded value.
...
10. **Curly braces in attribute value** — `{name}` inside a value breaks public reading of the entire record (`500 invalid input syntax for type json`), while in the admin panel everything looks fine; write placeholders as `%name%` (see `.claude/rules/admin-api.md`).
...
13. **Markup comment instead of fields by meaning** — the entire entry in one `text` attribute with HTML. It renders, and visually everything is correct; the price comes later and all at once: cannot search by field, cannot build a filter, cannot verify translation by slots, and the editor gets a rich-text window instead of a form. Comments are allowed **only for continuous prose**; everything that has a form — documents by years, contacts, schedules, numbers, people, stages — has fields by meaning, and this is resolved during model design, not when the client complains.
14. **Order of records hardcoded in code** — "let this card be first" is a data edit, not a layout edit: order changes in the CMS and in all locales, sorting in the component does not appear. And related values (icons, logos, photos) are searched **by the title of the record, not by its position**: binding by position looks functional only until the first rearrangement.
15. **Second mechanism where an existing one works** — for example, a new field for a logo when the directory already has a pair "marker + file": the front distinguishes the file type itself. Each extra mechanism the editor must know separately.
