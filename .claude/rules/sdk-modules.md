...
**Guest mode.** Cart/wishlist (`Users.getCart/...`), activity tracking (`UserActivity`) and recommendations Blocks work for **unauthorized guests** — the SDK sends the header `x-guest-id` instead of `Authorization`. For details, see `03-sdk-init.md` (section "Guest Mode").
...
The "team/experts" pattern: OneEntry admins with an assigned set of attributes work as content entities (masters, doctors, trainers) — photo, rating, services-`entity`, schedule-`timeInterval`. The "team member" indicator is a filled key attribute (for example, name): filter the list on your side, there is no separate flag.
...
> ⚠️ `getAllEvents` is public from the SDK side, but the events route must be **granted to the guest group** — it is not granted on some tenants, and the method returns `401` with the app token. Then look for event markers in the admin panel → Events, through the admin API (`GET /api/admin/events`, rule `admin-api`) or grant permission (`/admin-grant-permissions`). The existence of an event cannot be checked with trial `generateCode`/`checkCode` — they mask the error (see `rules/auth-provider.md`).

> Do not confuse with the **Subscriptions** module (paid subscriptions) — these are different entities. `Events.getAllSubscriptions` → subscriptions to products; `Subscriptions.getAllSubscriptions` → markers of paid subscriptions.
...
Paid subscriptions. `subscribe` returns `paymentUrl` for redirecting to payment (like `createSession` for orders). Skills: `/create-subscription`.
...
`ITrackActivity = { type: TUserActivityType, productId?, pageId?, categoryId?, query?, meta? }`. `TUserActivityType` = `'product_view' | 'page_view' | 'category_view' | 'search' | 'product_add_to_cart' | 'product_remove_from_cart' | 'product_add_to_wishlist' | 'product_remove_from_wishlist' | 'product_purchase' | 'product_rating'`. These events feed recommendations Blocks (recently-viewed, personal-recommendations, trending).
