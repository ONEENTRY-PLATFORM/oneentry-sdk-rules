...
**`rating` field** (aggregate rating) is now available in `IProductsEntity`, `IPagesEntity`, and `IUserEntity` — use it for stars on cards. Rating forms — form type `'rating'` (see `/create-reviews`).
...
**Device metadata (v1.0.155).** Each module has `setDeviceMetadata(value)` and `getDeviceMetadata()` — override the header `x-device-metadata` (the API binds refresh tokens to it); there is also the option `config.deviceMetadata`. Needed for server-side OAuth code exchange — see `03-sdk-init.md` and `/create-google-oauth`.
...
Content filter — customizable tree of nodes in the admin (pages, products, attributes, discounts, bonuses, payment methods). `IContentFilterItem.type` = `'page' | 'product' | 'admin' | 'attribute' | 'discount' | 'personal-discount' | 'bonus' | 'payment-method' | 'custom'`. Nodes are nested through `children`. Public (app-token).
...
> `IFormsEntity.type` is narrowed to `'order' | 'sing_in_up' | 'collection' | 'data' | 'rating' | null`. `IFormConfig` (element `moduleFormConfigs`) has received the field `exceptionIds?: string[]`.
...
>
> `searchPage` with `traficLimit: true` returns short cards `IPageSearchResult` (`{ id, title }`) — see the Products section, there is also a type narrowing pattern. Starting from v1.0.157, in this mode, the unnecessary pass for templates has been skipped (short cards do not have `templateIdentifier`).
>
> `getBlocksByPageUrl` enriches blocks with products (v1.0.153): in a block `type: 'product_block'` appears `products?: IProductsEntity[]`, in `type: 'similar_products_block'` — `similarProducts?: IProductsResponse` (`{ total, items }`); separate requests for block products are not needed. With `traficLimit: true` in the SDK config, enrichment is disabled, in case of a loading error, an empty array is placed in the field — access is only optional: `block.products ?? []`, `block.similarProducts?.items ?? []`.
...
Paid subscriptions. `subscribe` returns `paymentUrl` for redirect to payment (like `createSession` for orders). Skill: `/create-subscription`.
...
Reference of project entity types (`forPage`, `forProduct`, `forBlock`, …). Needed in admin scenarios — see `admin-api.md`.
...
The only content module with full CRUD — use it if the project stores data in OneEntry collections. The record body (`{ formIdentifier, formData }`) is structured like in `FormData`.
...
>
> ⚠️ `validateICollectionMarker` — the semantics are **opposite** to JSDoc: `true` = marker is FREE (does not exist), `false` = occupied. To check the existence of a collection, you need to use `getICollections()`, not this method. Details — `product-statuses.md` (there is also a similar case described).
...
```ts
import {
  expandAttributeTimeIntervals, // (attr, { from, to }) → TimeIntervalPair[]  — entire timeInterval attribute
...
> (it materialized a year of slots and inflated the cache). Public `Module._addTimeIntervalsToSchedules`
