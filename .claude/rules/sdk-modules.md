...
> **Rarely used modules** (`GeneralTypes`, `IntegrationCollections`, `Templates`, `TemplatePreviews`, `System`) are described at the end of the file. They are rarely needed in regular site code: only `IntegrationCollections` supports writing, and `System` is for internal use. Types of returned values are from `oneentry`.
...
**Other type clarifications (v1.0.158).** There is no breaking behavior — the API returned this before, only declarations changed, but now they can be relied upon without casts: `ILocalizeInfo.plainContent` (plain text next to `htmlContent`); `IOrderStatus` + `axis`/`isCancelFinal`/`isFinalSuccess`/`isMapped`; `IBaseOrdersEntity.totalSum` — **number** (for `IOrderByMarkerEntity` it remains a string); a form without fields returns `attributes: []`, not `{}` (normalized in `_normalizeAttr`); flags `IFormAttribute` (`isLogin`, `isSignUp`, …) and `IFormLocalizeInfo.title` have become optional; `IAttributeSchemaItem` + `position`/`listTitles`/`listType`/`moduleIdentifier`/`parentId`/`splitParts`, while `initialValue`/`isPrice` are optional; `IProductsResponse.totalFound?`; `IContentApiEvent.module` is optional; `IDiscountsEntity.attributeSetId` and `IDiscountValue.maxAmount` — `number | null`; `ITimeIntervalRange.period` — `number | null`; `ITimeIntervalSchedule.fullMonth`/`selectedYear` are optional; `IProductBlockSimilarRule` has been rewritten to a real form (`{ id, title, attributeMarker, conditionMarker, conditionValue, statusMarker, pageUrls }`).
...
| --- | --- | --- |
| One search string for the entire site | `Search.globalSearch(query, types?, …)` | names **and attribute values** of visible records, immediately across 18 types of entities |
...
Semantic search with v1.0.161 is available in seven modules: `Products` (was earlier) plus `Admins`, `Discounts`, `FormData`, `Orders`, `Pages`, `Users`. The form is the same for all — `body = { queryText, vectorDistanceThreshold?, maxHits?, debug? }` (`maxHits` — Top-K before pagination, default 100, maximum 500; `debug: true` adds a `distance` field to each element), the response is a container `{ items, total }`. The types of bodies (`IAdminsVectorSearch`, `IPagesVectorSearch`, …) and responses are imported from the root of the package: `import type { IAdminsVectorSearch } from 'oneentry'`. The vector index is enabled on the OneEntry project side — if it is not configured, the method will return `IError`, not an empty list; handle it through `isError` and degrade to regular search (see `.claude/rules/error-handling.md`).

**Other type clarifications (v1.0.161).** ⚠️ **Breaking:** the field `attributeSetIdentifier` has been removed from `ITemplatesPreviewEntity` (`TemplatePreviews`) — the API no longer returns it, code accessing it will stop compiling. Extensions without breaking: `IAccountsEntity.type` (`Payments`) is now `'stripe' | 'yookassa' | 'midtrans' | 'xendit' | 'custom'` — do not write `type === 'stripe' ? … : 'custom'`; `IFilterParams.conditionMarker` (`Products`) received operators `'pat'` (pattern match), `'same'` (exact value match), and `'same_part'` (exact part value match); `IContentFilterItem.value` (`Filters`) has become `string | string[] | null` — for range nodes this is an array of range values, so check `Array.isArray` before string operations; `IFormDataSearchEntity` (`FormData`) — a new type of records from vector search by form-data, it has moderation fields (`status`, `ip`, `fingerprint`, `isUserAdmin`, `userIdentifier`, `entityIdentifier`, `parentId`), which are not in `IFormDataEntity`.

**⚠️ Breaking: `Pages.getConfigPageByUrl` removed (v1.0.162)** along with the type `IPageConfig` — the API removed the route `GET /api/content/pages/{url}/config`, it responds with `404`. Take the catalog output settings from the `config` field of the page itself: `page.config.rowsPerPage`, `page.config.productsPerRow`.

**Tail `limit` for Blocks recommendations (v1.0.162).** Eight methods (`getCartComplement`, `getCartSimilar`, `getWishlistSimilar`, `getPersonalRecommendations`, `getRecentlyViewed`, `getRepeatPurchase`, `getTrending`, `getFrequentlyOrderedProducts`) have accepted an optional `limit` as the last argument. By default, it **is not set**: without it, the number set in the block settings in the admin panel applies — do not hardcode the number if the content manager should manage it.

**Attribute reading helpers (v1.0.163) — do not write your own.** The package exports parsing of `attributeValues`, which was previously written manually in each project:

```typescript
import { getAttributeFile, getAttributeFiles, getAdditionalFields } from 'oneentry'

const cover = getAttributeFile(product.attributeValues.img)      // IAttributeFile | null — first file
const gallery = getAttributeFiles(page.attributeValues.photos)   // IAttributeFile[] — all files
const blurDataURL = cover?.previewLink?.default?.[0]             // ready base64-LQIP
const extra = getAdditionalFields(product.attributeValues.price) // Record<marker, IAttributeValue>
```

`getAttributeFiles` collapses both forms of `value` (single object and array) into one list, `getAdditionalFields` removes the empty **array** that the API returns instead of an object when there are no nested fields. Nearby — type guards `isFileAttribute`, `isStringAttribute`, `isNumberAttribute`, `isListAttribute` (plus the existing `isTimeIntervalAttribute`). Details — `.claude/rules/attribute-values.md`.

**New types (v1.0.163).** `IAttributeFile` is now exported from the root of the package — do not redeclare it manually. `ITypedAttributeValue` — a discriminated union by `type` (`IFileAttributeValue`, `IStringAttributeValue`, `INumberAttributeValue`, `IListAttributeValue`, `ITimeIntervalAttributeValue`), narrowing `value` in `switch`; the union is intentionally closed, types outside it (`entity`, `date`, custom) remain on the usual `IAttributeValue`. `IAttributeValues` has not changed — the transition is optional.

**Validation of attribute values (v1.0.163–164).** `AttributeValuesSchema` replaced `z.record(z.any())` in the schemas of admins, blocks, menus, pages, products, templates: the file part (`downloadLink`, nested `additionalFields`) is checked, the rest remains open. An unfilled file attribute is returned by the API as `value: ""`, and with v1.0.164 an empty string is considered an empty value — previously, each such product wrote a warning in the log about an unexpected file form.

**`IFormAttribute.multiselect` declared (v1.0.164).** For a form field of type `list`, the flag determines how to render it: `true` — multiple options from `listTitles` (checkboxes), `false` — one (radio buttons). The data was coming earlier, but due to the index signature `[key: string]: unknown`, the field was read as `unknown`, so it was often overlooked. The same flag exists in `IAttributeSchemaItem` (v1.0.162).

**Device metadata (v1.0.155).** Each module has `setDeviceMetadata(value)` and `getDeviceMetadata()` — override the header `x-device-metadata` (the API binds refresh tokens to it); there is also the option `config.deviceMetadata`. Needed for server-side OAuth code exchange — see `.claude/rules/sdk-init.md` (section "Device metadata") and `/create-google-oauth`.
...
The "command/specialists" pattern: OneEntry admins with a designated set of attributes work as content entities (masters, doctors, trainers) — photo, rating, services-`entity`, schedule-`timeInterval`. The "team member" sign is a filled key attribute (for example, name): filter the list on your side, there is no separate flag.
...
- `getAttributesByMarker` returns an array of **attributes** (`IAttributesSetsEntity[]`, without `id`/`identifier`/`schema`), not sets. Since v1.0.158 this has been reflected in the types; until 1.0.157 inclusive, d.ts declared `IAttributeSetsEntity[]` — do not trust the declared type here on the old SDK.
...
> v1.0.157: in `ISignUpEntity` (response from `signUp`) `attributeSetId` and `attributesSets` have been declared — the API returned them, but with `validation.enabled` they were cut from the response.
...
// Recommendations / personalization (signPrice? — marker of the order storage for price fixation:
// products are returned with the signed price signedPrice, which is passed in products[] of the order)
getFrequentlyOrderedProducts(productId, marker, langCode?, signPrice?, limit?): IProductsResponse
getCartComplement(marker, langCode?, signPrice?, limit?): IProductsResponse
getCartComplementByProductIds(marker, body: IBlockProductsLookup): IProductsResponse
getCartSimilar(marker, langCode?, signPrice?, limit?): IProductsResponse
getCartSimilarByProductIds(marker, body: IBlockProductsLookup): IProductsResponse
getWishlistSimilar(marker, langCode?, signPrice?, limit?): IProductsResponse
getWishlistSimilarByProductIds(marker, body: IBlockProductsLookup): IProductsResponse
getPersonalRecommendations(marker, langCode?, signPrice?, limit?): IProductsResponse
getRecentlyViewed(marker, langCode?, signPrice?, limit?): IProductsResponse
getRepeatPurchase(marker, langCode?, signPrice?, limit?): IProductsResponse
getTrending(marker, langCode?, signPrice?, limit?): IProductsResponse
...
> ⚠️ **Breaking (v1.0.157):** `subscribeByMarker`, `unsubscribeByMarker`, `subscribeToForm`, `unsubscribeFromForm` now truly return `IError` on API refusal. Until 1.0.157, the general helper only caught exceptions, and with `isShell: true` (default), the SDK **returns** the error instead of throwing it — thus, any refusal was reported as `true`. Check strictly: `if (result === true)`, `if (result)` will skip the error object. The same pitfalls were present in `Subscriptions.cancelSubscription` / `recoverSubscriptions`.
>
> ⚠️ `getAllEvents` is public from the SDK side, but the events route must be **issued to the guest group** — it is not issued for some tenants, and the method returns `401` with the app token. Then look for event markers in the admin panel → Events, through the admin API (`GET /api/admin/events`, rule `admin-api`) or grant permission (`/admin-grant-permissions`). The existence of an event cannot be checked with trial `generateCode`/`checkCode` — they mask the error (see `.claude/rules/auth-provider.md`).
>
...
Content filter — a customizable tree of nodes (pages, products, attributes, discounts, bonuses, payment methods). `IContentFilterItem.type` = `'page' | 'product' | 'admin' | 'attribute' | 'discount' | 'personal-discount' | 'bonus' | 'payment-method' | 'custom'`. Nodes are nested through `children`. Public (app-token).
...
> `IFormsEntity.type` narrowed to `'order' | 'sing_in_up' | 'collection' | 'data' | 'rating' | null`. `IFormConfig` (element `moduleFormConfigs`) received the field `exceptionIds?: string[]`.

## FormData

```ts
postFormsData(body: IBodyPostFormData, langCode?): IPostFormResponse
getFormsDataByMarker(marker, formModuleConfigId, body?: IFormsDataFilter, isExtended?, langCode?, offset?, limit?): IFormsByMarkerDataEntity
...
**Filter `getFormsDataByMarker` is typed (v1.0.163)** — `IFormsDataFilter` = `{ entityIdentifier?, parentId?, userIdentifier?, status?: FormDataStatus[], dateFrom?, dateTo? }`; previously it was just `object`, and no one caught the typo. Three traps:

- **The API silently ignores unknown fields.** `statuses: ['approved']` instead of `status` will return **all** records, including unmoderated ones — the code looks correct. The field `entityparentIdentifier` does not exist: responses for records are selected through `parentId`.
- **`status` — only an array** and only from the set `sent | moderation | approved | banned | deleted` (type `FormDataStatus`). A string instead of an array or another value → `400 each value in status must be a valid enum value`.
- **`formData` comes wrapped in language.** The raw response from the API — `formData: { "en_US": [ … ] }`; the SDK unwraps it and returns an array under the requested `langCode`. Code written based on the response from curl or the admin panel reads `formData[langCode]`, gets `undefined`, and renders an empty list without errors.

> `IBodyTypeImageGroupOfImages.type` until v1.0.163 contained a typo `'groupOfImage'` — a value that the API rejects (`400 formData's marker '…' type must be one of […]`), and the editor auto-completed it. The correct value is `'groupOfImages'`. The form of the attached file is `IImageValue` / `IFileValue`, and this is **not** what `FileUploading.upload` returns (there is `contentType`, which form-data rejects).
...
>
> `searchPage` with `traficLimit: true` returns short cards `IPageSearchResult` (`{ id, title }`) — see the Products section, there is also a type narrowing pattern. Since v1.0.157, this mode has skipped the useless pass for templates (short cards do not have `templateIdentifier`).
>
> `getBlocksByPageUrl` enriches blocks with products (v1.0.153): for a block `type: 'product_block'` appears `products?: IProductsEntity[]`, for `type: 'similar_products_block'` — `similarProducts?: IProductsResponse` (`{ total, items }`); separate requests for products of the block are not needed. With `traficLimit: true` in the SDK config, enrichment is disabled, and in case of loading error, the field is filled with `[]` — access is only optional: `block.products ?? []`, `block.similarProducts?.items ?? []`.
...
`body: IFilterParams[]` — a required parameter, but by default `[]`. If filters are not needed, it can be omitted.
...
- Per-method query types (v1.0.154), all exported from the SDK: base `IProductsQueryBase = { offset?, limit?, sortOrder?: 'DESC'|'ASC', sortKey?: 'id'|'position'|'title'|'date'|'price', signPrice? }` — for `getProducts` / `getProductsEmptyPage` / `getProductsByPageId` / `getProductsByPageUrl`. For `getRelatedProductsById` — `IProductsRelatedQuery` (base + `statusMarker?`, `templateMarker?`); for `getProductsPriceByPageUrl` — `IProductsPriceQuery` (base **without** `sortKey`, + `statusMarker?`); for `getProductsByIds` — `IProductsByIdsQuery` (only `signPrice?`: pagination and sorting this endpoint no longer accepts, extra fields — TS error). `IProductsQuery` — deprecated alias `IProductsQueryBase`, do not use in new code.
- `getProductsByVectorSearch` — `body: IVectorSearchProducts = { queryText, vectorDistanceThreshold?, maxHits?, debug? }`. Semantic search by the meaning of the request (not by substring, like `searchProduct`).
- `getProductsEmptyPage` — now **POST**, returns `IAggregatedProductGroup[]` (`{ attrValue, items, productIds, total }`), not `IProductsResponse`.
- **Quick search and `traficLimit` (types clarified in v1.0.157).** `searchProduct` → `IProductsEntity[] | IProductSearchResult[] | IError`, `Pages.searchPage` → `IPagesEntity[] | IPageSearchResult[] | IError`. With `traficLimit: true`, it returns the **raw response of quick search** — short card `IProductSearchResult = { id, title, pageId }` / `IPageSearchResult = { id, title }`, without `attributeValues`, `localizeInfos`, `blocks`. The behavior at runtime has not changed — previously the signature simply lied, and `attributeValues` in traficLimit mode silently came as `undefined`. Narrow down by config or by field:
...
- ⚠️ **`offset`/`limit` — drilldown mode**: the API accepts them only if exactly one available type is passed in `types`, otherwise it responds with `400`. Without them, each group returns in full, and the SDK does not substitute anything by default. Pagination of output "across all types at once" is impossible by design: first a general request, then a drilldown by the selected type.
- `visibility` defaults to `'all'` — for a public site, pass `'visible'`, otherwise hidden records will be included in the output.
- The element carries **the context of the match**, not the entity itself: `matchKind` (`'exact' | 'title' | 'attributeName' | 'attributeValue'`), `matchedField`, `fragment` (plain text around the match, without markup — highlight it yourself), `langCode`, plus `parent` for entities without their own page (slide → block, order → storage) and `matchedAttribute`. `attributeValues`, `localizeInfos`, and prices are **not there** — for cards go to `Products.getProductsByIds` by collected `id`.
- `id` — `number | string`: for `workflows` and `attributes` it is string. Do not blindly convert to a number.
...
>
> ⚠️ **Breaking (v1.0.157):** `cancelSubscription` / `recoverSubscriptions` no longer respond `true` on API error — with `isShell: true` (default) it returns `IError`. Check only with strict `if (result === true)`: the error object is also truthy.
...
> v1.0.157: in `ICollectionRow` `langCode` and `formIdentifier` have been declared (`formIdentifier` comes on update) — the API always returned them, but with `validation.enabled` they were cut from the response.
>
> ⚠️ `validateICollectionMarker` — the semantics are **opposite** to JSDoc: `true` = marker is FREE (does not exist), `false` = occupied. Check for the existence of a collection through `getICollections()`, not this method. Details — `product-statuses.md` (there is also a similar case described).

## System

```ts
getApiStat(): unknown          // number of requests to the API
test404(): unknown             // test error handling — returns 404
test500(): unknown             // test error handling — returns 500
```

Service module. `test404`/`test500` — only for checking your error handling (see `error-handling.md`), there is no need to use them in production site code.
