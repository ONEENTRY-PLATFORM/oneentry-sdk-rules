...
paths:
  ...
  - "src/components/**/*.tsx"
---
# Front Security on OneEntry — CSP, Headers, CMS-HTML Sanitization
...
- **The customer's session lives in `localStorage`**, not in an httpOnly cookie — the refresh token is tied to the browser's `x-device-metadata`, and server-side storage breaks rotation (see `.claude/rules/tokens.md`).

Both decisions are correct, but together they mean: **any script that executes on the page can take the customer's session**. This rule is about how this is compensated.

---

## 1. CMS-HTML Sanitization — mandatory

`localizeInfos.htmlContent` (pages, blocks) and `attributeValues.*.value.htmlValue` (attributes of type `text`) — this is HTML from the admin panel editor. It goes into `dangerouslySetInnerHTML`, and this is the **only** channel through which foreign markup enters the DOM of the storefront.

**Threat model** — a compromised or unscrupulous content manager account. The surface is narrow, but the cost is the customer's session from `localStorage`.
...
**Why `class` and `style` are outside the list:** they do not execute scripts, but on a Tailwind storefront `class="fixed inset-0 z-50"` — a ready clickjacking overlay over the payment button. The same goes for `style` with `position:fixed`.
...
1. **URL schemes are decoded by the browser before checking.** `java&#09;script:` and `&#106;avascript:` execute. Normalize entities and control characters **before** checking the scheme, and discard `javascript:`, `vbscript:`, `data:`, `blob:`, `file:`, `about:`.
   ...
   ```typescript
   // NUL, tab, newline, spaces are ignored by the URL parser: `java	script:` — a working payload
...
3. **Duplicate attribute.** `href="ok" href="javascript:…"` — browsers take the **first** occurrence; repeat this rule, otherwise the verified value will lose to the unverified one.
...
Escape `<` and `"`. Do not touch `&` — the editor already outputs entities, and re-escaping will show `&amp;nbsp;` in the text.

> The sanitizer is an ideal candidate for unit tests on fixtures: see `.claude/rules/unit-testing.md`.

---

## 2. CSP — not about banning scripts, but about leak channels

`script-src` in the App Router **must** contain `'unsafe-inline'`: RSC payload is streamed as inline `<script>`. The nonce alternative requires per-request rendering — that is, it kills ISR across the entire site. Do not try to "fix" this.
...
/** Tenant origin — from env, so the policy follows the project, not hardcoded. */
...
Key directives — `connect-src`, `img-src`, `form-action`, `base-uri`: they prevent injected code from sending the token to a foreign host. `wss://` is needed if the `WS` module (event subscriptions) is used.
...
const nextConfig: NextConfig = {
  poweredByHeader: false,   // do not expose the stack
  async headers() {
...
## 5. What is consciously absent here
...
4. CSP is built from env, `connect-src` includes the tenant's origin and `wss://` when using WS.
5. `poweredByHeader: false`, five headers from section 3 are returned on all routes.
6. E2E spec checks headers **and absence of CSP violations in the console** — CSP that silently cuts off fonts is worse than absent (see `.claude/rules/playwright-e2e.md`).
