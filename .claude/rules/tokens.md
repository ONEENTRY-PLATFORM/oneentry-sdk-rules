...
//   token. The spurious 401 is no longer present (≥ 1.0.152), but this is an unnecessary round-trip.
...
> Related rules:
>
> - `.claude/rules/performance-rtk.md` — `pollingInterval` for `getMe`, the `reDefine` pattern within the RTK Query auth flow, protection against race conditions when updating the token with parallel RTK requests.
