...
Quick reference for key concepts. If you're unsure about a term, check here.
...
- `locale` — string from Next.js params (`'en_US'`, `'ru_RU'`)
- `langCode` — the same, parameter for SDK methods
...
Callback in the SDK config that is called **automatically** on every rotation of `refreshToken`.
...
**NEVER MAKE UP the data structure!** Even if examples in the documentation look different - check the real TypeScript types.
**NEVER MAKE UP DATA! Always obtain from the API (Pages, Menus, Products, Blocks, and other entities). If you don't know where to get the data → ASK THE USER. This is CRITICALLY IMPORTANT!**
...
**🚨 CRITICALLY IMPORTANT:** When a prompt contains multiple tasks ("do X + add Y + create Z"), there is a risk of missing rules in haste. **Real case:** in the prompt "create product pages + profile + authorization" AI missed the flag `isCheckCode: true` and did not add the step `activateUser()` — the rule was in `.claude/rules/auth-provider.md`, but was not applied due to haste.
...
- After **each** subtask — run the checklist: have all rules from `.claude/rules/*.md` been applied? Have all fields from the API been processed? Have all provider flags (`isCheckCode`, `systemCodeTlsSec`, etc.) been taken into account?
- Only after explicit verification — proceed to the next subtask

**Option B — In parallel through agents (for independent tasks):**

- If the subtasks are **completely independent** (different pages, different components without shared dependencies) — run several agents in parallel through the Agent tool (`general-purpose` or `Explore`)
...
| reviews, reviews, product ratings | `/create-reviews` |
...
> ⚠️ Trigger word found → **first skill**, then code. Do not write the component manually.
...
1. ☑️ **Where is the data from?**
   ...
   - DO NOT make up the data source!
2. ☑️ **Have you checked the types in the SDK?**
   ...
   - DO NOT make up the data structure - check real TypeScript types!
...
7. ☑️ **Is data transformation needed?**
   - Is the data from the API already in the desired format? → Use it directly
...
8. ❓ **There are several options for solving**
...
4. **🎨 COPY LAYOUT EXACTLY: If the user provided the layout (HTML/JSX), copy it exactly, especially if the same framework is used (e.g., Tailwind CSS). Do not change classes, structure, and styles without explicit necessity. Only replace hardcoded data with data from the API.**
...
13. **When creating pages, get content from CMS Pages, not hardcode**
...
### IMPORTANT: API permissions and record count limitations
...
3. This will allow you to get **all entities without limits** on the number
...
The SDK is isomorphic — it works on both the server and the client. The choice of context depends on the rendering strategy:
...
// saveFunction — called by the SDK automatically on every rotation of refreshToken
...
3. **Call from the client** via `getApi()` after `reDefine()` — token is managed automatically by `saveFunction`
4. **Always get fresh data before writing** — call `getUser()` just before `updateUser()`, do not use the previously cached user object: between getting and writing, another code may have changed `state` (for example, `cart` or `favorites`)
...
// Writing a single field — always get fresh user inside — do not pass it as a parameter
export async function saveUserFavorites(favorites: number[]) {
  // ✅ Get FRESH data just before writing — do not use cached user
  const user = (await getApi().Users.getUser()) as IUserEntity;
  if (isError(user)) return;
  await getApi().Users.updateUser({
    formIdentifier: user.formIdentifier,
    state: { ...user.state, favorites }, // spread the current state — cart and other fields are preserved
  });
}

// ❌ INCORRECT — user could have been obtained a long time ago, state.cart has already changed
export async function saveUserFavoritesWrong(favorites: number[], cachedUser: IUserEntity) {
  await getApi().Users.updateUser({
    ...
    state: { ...cachedUser.state, favorites }, // will overwrite the current changes in cart!
  });
}
```
...
- If you need to reset state when a dependency changes — pass the initial value directly into `useState`, not through an effect
...
## Common Hallucinations of AI (real examples of errors)
...
### Invented `children` field in the menu
...
Get colors and price ranges from the API, do not hardcode. The full pattern for the catalog with filters — skill **`/create-product-list`**.
...
> The SDK normalizes data before returning: removes locale wrapping from `attributeValues` and `localizeInfos`, converts `additionalFields` from an array to `Record<marker, field>`, transforms single `image.value` from an array to an object. Curl returns raw data — code based on it will contain errors.
